...
class Propagator:
    ...
    def add_house(self, house:House):
        if self.is_house_present(house.house_no):
        ...
        if self.power_remaining >= house.power_required:
            ...
            house.assign_propagtor(self.propagator_no)
    ...
    def remove_house(self, house_no:int):
        if self.is_house_present(house_no):
            v = self.is_house_present(house_no)
            self.houses_connected.remove(v)
            return True
        else:
            return False
      #  pass
...
class PowerGrid:
    ...
    def add_propagator(self, propagator:Propagator):
        ...
        if propagator.propagator_no in self.propagators:
            print("propagator already exist")
            return
        else:
    ...
    def remove_propagator(self, propagator_no):
        if not propagator_no in self.propagators:
            print("Propagator not exist")
            return
